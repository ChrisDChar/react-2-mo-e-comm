    ...
    <section className="max-w-7xl mx-auto px-4 pt-34 py-12 grid grid-cols-1 md:grid-cols-2 gap-12">
      <div>
        <div className="relative w-full h-[400px] border border-gray-200 flex items-center justify-center overflow-hidden rounded-lg mb-4">
          ...
          <button
            onClick={handlePrev}
            className="absolute left-3 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow cursor-pointer z-20"
          >
            <AiOutlineLeft />
          </button>
          ...
          <button
            onClick={handleNext}
            className="absolute right-3 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow cursor-pointer z-20"
          >
            <AiOutlineRight />
          </button>
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto">
            {product.images.map((img, idx) => (
              <img
                ...
                src={img}
                alt={`thumb-${idx}`}
                className={`w-20 h-20 object-contain border cursor-pointer rounded ${
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-4 mt-4">
          <div className="flex items-center border rounded overflow-hidden">
            <button className="px-3 py-1 cursor-pointer" onClick={decrementQty}>
              -
            </button>
            <span className="px-4 py-1">{quantity}</span>
            <button className="px-3 py-1 cursor-pointer" onClick={incrementQty}>
              +
            </button>
          </div>
